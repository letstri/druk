...
- **Custom**: `allowScrollPastEnd` (`src/ui/EditorPane.tsx:348`), ~60 lines rewriting
...
- **Custom**: `scrollByRows` / `scrollToRow` / `scrollTo` (`src/ui/EditorPane.tsx:1626`),
...
- **Custom**: `scrollMetrics` / `scrollbar()` / `dragTo` (`src/ui/EditorPane.tsx:769`,
...
- **Custom**: `enlargeThumb` (`src/ui/list.ts:39`), patching `slider.getVirtualThumbSize`.
...
- **Custom**: `reveal` (`src/ui/list.ts:113`) — a `setTimeout(0)` then a manual
...
- **Custom**: `selectOnMultiClick` (`src/ui/EditorPane.tsx:419`) and the same counting
  in `src/ui/FileTree.tsx:120`.
...
### A8. Selection confined to the editor
- **Custom**: `allowSelectionOnlyInEditor` (`src/ui/EditorPane.tsx:265`), ~12 lines
  patching the renderer's public `startSelection` once per renderer.
...
- **Custom**: `cut` and `wrapText` (`src/ui/text.ts`, 39 lines).
...
- **Custom**: `struckThroughId` (`src/languages/highlight.ts:239`) — a direct
...
- **Custom**: `styleIdOver` (`src/languages/highlight.ts:278`) — registers one style per
...
- **Custom**: `inCells` (`src/editor/columns.ts`, 33 lines).
...
### A13. Deleting lines
- **Custom**: `removeLines` and friends (`src/editor/lines.ts`, 78 lines).
...
- **Custom**: `setSelection(0, value.length)` in `onMount` (`src/ui/TextInput.tsx:33`).
...
- **Custom**: `useListKeys` (`src/ui/list.ts:145`) plus each picker's own row rendering.
...
- **Custom**: `afterResize` (`src/ui/EditorPane.tsx:288`), patching protected `onResize`.
...
- **Custom**: `ignoreScrollOutsideBounds` (`src/ui/EditorPane.tsx:303`).
...
| Custom | Feature lost |
| --- | --- |
| `src/editor/folds.ts` (299 lines) | code folding, and the blank-row gap the review card is drawn in — the edit buffer has no hidden line |
...
| `DiffView` hatch (`src/ui/DiffView.tsx:141`) | the padded side of a split diff reads as blank editor — a terminal has no fill patterns |
