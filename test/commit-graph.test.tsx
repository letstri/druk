...
test('lanes come out as box drawing, one colour per lane', () => {
  ...
  expect(laneSpans('|\\', palette).map((span) => span.color)).toEqual([
    'red',
    'green',
  ])
  expect(laneSpans('* | |', palette).map((span) => span.color)).toEqual([
...
test('reading a commit and closing it leaves the graph where it was', async () => {
  const t = await launch(repo(20), {}, { height: 24, width: 120 })
  await runCommand(t, 'Commit graph')
  ...
  await press(t, (i) => i.pressKeys([String.fromCodePoint(23)]))
