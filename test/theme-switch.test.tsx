...
test("switching themes never leaves a previous theme's colors behind", () => {
...
test('plain identifiers stay readable against the background in every theme', async () => {
  ...
  for (const name of themeNames()) {
    ...
    const groups = [
      ...(style as unknown as { getAllStyles: () => Map<string, unknown> })
        .getAllStyles()
        .keys(),
    ]
    ...
    for (const segment of segments) {
      ...
      const fg = group
        ? (themeFor(name).syntax[group] as { fg?: string })?.fg
        : undefined
      if (!fg) {
        continue
      }
