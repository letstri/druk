...
test('scrolling deep into a large file keeps highlights', async () => {
  ...
  expect(foreground).toContain(
    rgb((THEMES.dark.syntax.property as { fg: string }).fg)
  )
}, 20_000)
...
test('segmenting one line costs a fraction of segmenting the whole file', async () => {
  const source = `${Array.from(
    { length: 8000 },
    (_, i) =>
      `export function fn${i}(a: number, b: string): string { return \`x-${i}\` }`
  ).join('\n')}\n`

  const parsed = await parseHighlights(source, 'typescript')

  const whole = time(5, () => segmentsIn(parsed, 0, WHOLE))
  let line = 4000
  const one = time(50, () => {
    const at = line
    line += 1
    return segmentsIn(parsed, at, at)
  })

  expect(
    `${whole.toFixed(2)}ms whole vs ${one.toFixed(3)}ms per line: ${whole / one > 20}`
  ).toBe(`${whole.toFixed(2)}ms whole vs ${one.toFixed(3)}ms per line: true`)
}, 30_000)
