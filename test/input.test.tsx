  ...
  for (const line of capture.lines) {
    for (const span of line.spans) {
      if (span.text.includes(needle) && span.fg && span.bg) {
        return { bg: at(span.bg), fg: at(span.fg) }
      }
    }
  }
  return null
...
test('typed text is readable in the palette, search and prompts', async () => {
  for (const theme of Object.keys(THEMES) as ThemeName[]) {
    ...
    await openPalette(t)
    await press(t, (i) => i.typeText('zzz'))
    const palette = spanColors(t, 'zzz')
    expect(`${theme} palette:${palette !== null}`).toBe(`${theme} palette:true`)
    expect(
      `${theme} palette contrast>60:${contrast(palette!.fg, palette!.bg) > 60}`
    ).toBe(`${theme} palette contrast>60:true`)
    ...
    expect(
      `${theme} search contrast>60:${contrast(search!.fg, search!.bg) > 60}`
    ).toBe(`${theme} search contrast>60:true`)
  }
}, 30_000)
