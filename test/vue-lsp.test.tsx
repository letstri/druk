...
test('a .vue file is served by the vue server and by a tsserver beside it', () => {
  ...
  const tsserver = resolveServers('vue', {}).find(
    (server) => server.id === 'vue-typescript'
  )
...
test("the plugin location is a prefix, the project ahead of druk's own copy", () => {
  ...
  mkdirSync(join(installed, 'node_modules', VUE_TYPESCRIPT_PLUGIN), {
    recursive: true,
  })
  expect(vuePluginLocation(project, installed)).toBe(installed)

  mkdirSync(join(project, 'node_modules', VUE_TYPESCRIPT_PLUGIN), {
    recursive: true,
  })
...
test('a tsserver/request is relayed to the server that drives one', async () => {
  ...
  await press(t, (input) => input.typeText('hy'))
...
test('a tsserver/request nobody can answer is still answered', async () => {
  ...
  const t = await launch(
    ...
    {
      lsp: true,
      lspServers: { vue: [process.execPath, HYBRID], 'vue-typescript': [] },
    },
    {},
    { openFile: join(dir, 'a.vue') }
  )
...
test('an edit still reaches the live server after a sibling dies', async () => {
  ...
  await press(t, (input) => input.typeText('ab'))
...
test('the item resolved is the one the answering server holds', async () => {
  ...
  const t = await launch(
    dir,
    {
      ...
      lspServers: {
        vue: [
          process.execPath,
          join(import.meta.dir, 'fixtures', 'silent-lsp.ts'),
        ],
        'vue-typescript': [
          process.execPath,
  ...
  await press(t, (input) => input.typeText('drukLazy'))
