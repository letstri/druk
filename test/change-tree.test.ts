...
test('a collapsed folder keeps its row and hides its subtree', () => {
  ...
  expect(rows.find((row) => row.kind === 'dir')).toMatchObject({
    collapsed: true,
    files: 2,
  })
})
...
test('a heading with nothing under it is not drawn', () => {
  expect(shape(changeRows(changes('a.ts'), 'list'))).toEqual([
    'section:Changes@0',
    'file:a.ts@1',
  ])
})
...
test('one path half-staged is a row under each heading', () => {
  ...
  expect(
    shape(changeRows(list, 'list', new Set([foldKey('staged', '')])))
  ).toEqual(['section:Staged Changes@0', 'section:Changes@0', 'file:a.ts@1'])
})
...
test('a heading stands for every change in it, a folder for its subtree', () => {
  ...
  const rels = (at: number) =>
    changesFor(list, rows[at]!).map((change) => change.rel)
