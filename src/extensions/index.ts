...
function manifestsIn(dir: string): string[] {
  ...
  return entries
    .filter((entry) =>
      ...
      entry.isDir
        ? true
        : entry.name.endsWith('.json') && entry.name !== 'index.json'
    )
    .map((entry) =>
      entry.isDir ? join(dir, entry.name, MANIFEST) : join(dir, entry.name)
    )
    .toSorted()
}

// A signal: the settings page lists these, so a reload has to repaint it.
const [loaded, setLoaded] = createSignal<ExtensionLoad>({
  extensions: [],
  problems: [],
})
...
export function loadExtensions(
  ...
  for (const source of [
    ...manifestsIn(userDir),
    ...manifestsIn(projectExtensionsDir(rootDir)),
  ]) {
