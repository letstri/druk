  ...
  win32: [
    {
      ...
      read: (stdout, ok) => {
        ...
        const value = /AppsUseLightTheme\s+REG_DWORD\s+0x([0-9a-f]+)/iu.exec(
          stdout
        )?.[1]
        if (value === undefined) {
          return null
...
function runProbe(probe: Probe): Promise<Appearance | null> {
  const { promise, resolve } = Promise.withResolvers<Appearance | null>()
...
export function watchAppearance(
  ...
  const poll = () => {
    ...
    inflight = true
    void (async () => {
      const now = await detectAppearanceAsync()
      inflight = false
      report(now)
    })()
  }
