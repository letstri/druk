...
export function renderSequence(diagram: SequenceDiagram): Line[] {
  ...
  if (participants.length === 0) {
    return [[{ role: 'muted', text: 'sequenceDiagram' }]]
  }

  const index = new Map(
    participants.map((participant, at) => [participant.id, at])
  )
  ...
  for (const [at, box] of boxes.entries()) {
    if (at > 0) {
      x += gaps[at - 1]! + Math.ceil(boxes[at - 1]! / 2) + Math.ceil(box / 2)
    }
  ...
  const shift = Math.max(
    0,
    ...centres.map((centre, at) => Math.floor(boxes[at]! / 2) - centre)
  )
  ...
  for (const [at, participant] of participants.entries()) {
  ...
  const bodyTop = top + 3
  const rows = diagram.events.reduce(
    (total, event) => total + rowsFor(event),
    0
  )
  ...
  for (const event of diagram.events) {
    if (event.type === 'block') {
      ...
      const label = event.closing
        ? 'end'
        : `${event.keyword} ${event.text}`.trim()
    ...
    const head =
      event.head === 'cross' ? '✕' : event.head === 'circle' ? '○' : null
    ...
    if (text) {
      ...
      const at =
        Math.min(start, end) + Math.max(0, Math.floor((span - width(text)) / 2))
