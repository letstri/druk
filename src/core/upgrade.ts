...
const MANAGER_PATHS: readonly (readonly [PackageManagerName, RegExp])[] = [
  ['pnpm', /[/\\](?:\.?pnpm|pnpm-global)[/\\]/u],
  ['bun', /[/\\]\.bun[/\\]/u],
  ['yarn', /[/\\]\.?yarn[/\\]/u],
  ['deno', /[/\\]\.deno[/\\]/u],
...
export function upgradeCommand(install: Install): string {
  ...
  if (install.kind === 'script') {
    return 'curl -fsSL https://druk.sh/install | bash'
  }
...
const DESCRIPTION: Record<InstallKind, (install: Install) => string> = {
  ...
  package: (install) =>
    `Updating the global ${install.manager ?? 'npm'} install.`,
...
const packageNames = (arch: string) =>
  arch === 'arm64'
    ? 'the arm64 .deb or aarch64 .rpm'
    : 'the amd64 .deb or x86_64 .rpm'

const SPINNER = [...'⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏']

// The install script and brew both report a no-op on stdout; neither uses an exit code for it.
export const changedNothing = (output: string) =>
  /already (?:installed|up-to-date)/iu.test(output)
...
export async function withSpinner<T>(
  ...
  const draw = () => {
    ...
    write(`\r\u001B[2K${SPINNER[frame % SPINNER.length]} ${label}  ${seconds}s`)
    frame += 1
  }
  write('\u001B[?25l')
  draw()
...
export async function runUpgrade(
  ...
  write(
    `${DESCRIPTION[install.kind](install)}\n$ ${command}\n${live ? '' : '\n'}`
  )
  try {
    ...
    const { code, output } = live
      ? await withSpinner(write, 'Updating', run)
      : await run()
    if (code === 0) {
      if (live) {
        write(
          changedNothing(output) ? '✓ Already up to date.\n' : '✓ Updated.\n'
        )
      }
    } else {
      if (output.trim()) {
