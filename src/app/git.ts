...
export function createGit(
  ...
  const [commitVariant, setCommitVariant] =
    createSignal<CommitVariant>('commit')
          ...
          return [
            {
              ...
              path,
              rel,
              status: entry.unstaged ?? 'modified',
            },
          ]
        }
        const both: Change[] = []
        if (entry.staged) {
          both.push({ area: 'staged', path, rel, status: entry.staged })
        }
        if (entry.unstaged) {
          both.push({ area: 'unstaged', path, rel, status: entry.unstaged })
        }
        return both
      })
  ...
  const rows = createMemo(() => [
    ...
    ...(staging()
      ? commitRows(syncCommits().incoming, syncCommits().outgoing, collapsed())
      : []),
  ])
  ...
  const cursorRepo = createMemo(() => {
    ...
    return repoFor(
      row.kind === 'file' ? row.change.path : join(rootDir, row.rel)
    )
  })
  ...
  const typeMessage = (value: string) => {
    ...
    const at = messageAt()
    if (at >= 0 && value !== messageHistory()[at]) {
      setMessageAt(-1)
    }
  }
...
export function createGitOp(deps: {
  ...
  return (
    ...
    options: {
      ...
      touchesTree?:
        | { kind: 'sync' }
        | { kind: 'followDisk'; paths: readonly string[] }
    ...
    if (git.gitBusy()) {
      return status.say(
        'A git command is already running — let it finish',
        'warn'
      )
    }
    status.whileFree(() => {
      ...
      void (async () => {
        try {
          ...
          git.bump()
          if (!result.ok) {
            if (options.handleFailure?.(result)) {
              return
            }
          ...
          if (options.touchesTree?.kind === 'sync') {
            ...
            if (warning) {
              return status.say(warning, 'warn')
            }
          } else if (options.touchesTree?.kind === 'followDisk') {
            for (const path of options.touchesTree.paths) {
              workspace.followDisk(path)
            }
          }
          return status.say(
            options.done
              ? options.done(result)
              : result.detail || `${verb} done`
          )
        } finally {
...
export function runCommit(
  ...
  gitOp(
    ...
    async (repo) => {
      const paths =
        opts.paths === 'all' ? [...statusMap(repo).keys()] : opts.paths
      if (paths !== null && paths.length === 0) {
        return { detail: 'Nothing to commit — working tree clean', ok: false }
      }
      const commit =
        paths === null
      ...
      if (!branch) {
        return { detail: 'Committed — no branch to push', ok: true }
      }
      const upstream = await upstreamOf(repo)
      const hasUpstream =
        upstream?.name !== null && upstream?.name !== undefined
      pushed = { branch, hasUpstream }
      if (opts.variant === 'commitPush') {
        return push(repo, branch, hasUpstream)
      }
      return hasUpstream
        ? pullAndPush(repo, branch, true)
        : push(repo, branch, false)
    },
    {
      done: (result) => {
...
async function ignoredIn(
  repos: string[],
  paths: string[]
): Promise<Set<string>> {
  const ignored = new Set<string>()
  for (const [repo, group] of groupByRepo(paths, repos)) {
    for (const path of await ignoredAmongAsync(repo, group)) {
      ignored.add(path)
    }
  }
  return ignored
...
export function wireGitEffects(deps: {
  ...
  const refreshLines = deferred(() => {
    ...
    void (async () => {
      const lines = await diffLines(path, git.diffBase())
      if (run === linesRun) {
        git.setGitLines(lines)
      }
    })()
  })
  ...
  const refreshUpstream = deferred(() => {
    ...
    void (async () => {
      ...
      const [incoming, outgoing] =
        repo && upstream?.name !== null && upstream?.name !== undefined
          ? await Promise.all([
          ...
          : [[], []]
      if (run === upstreamRun) {
        git.setSyncCommits({ incoming, outgoing })
      }
    })()
  })
  ...
  createEffect(
    on(
      () => [git.branch(), git.revision(), git.activeRepo()] as const,
      refreshUpstream
    )
  )
  ...
  const currentRepos = (depth: number) => {
    const now = Date.now()
    if (depth === scannedDepth && now - scannedAt < SCAN_INTERVAL) {
      return scanned
    }
    scannedDepth = depth
    scannedAt = now
  ...
  const refreshStatus = deferred(() => {
    ...
    void (async () => {
      const maps = await Promise.all(
        git.repos().map((repo) => statusEntriesAsync(repo, base))
      )
      if (run !== generation) {
        return
      }
    ...
    if (config.respectGitignore) {
      ...
      void (async () => {
        ...
        if (run === generation) {
          git.setGitIgnored(ignored)
        }
      })()
    }
    const active = git.activeRepo()
    void (async () => {
      const branch = active ? await currentBranchAsync(active) : null
      if (run === generation) {
  ...
  createEffect(
    on(
      ...
      ([path, selected]) =>
        git.setEditorRepo(
          (path ? git.repoFor(path) : null) ??
            (selected ? git.repoFor(selected) : null)
        )
    )
  )
}
