...
export function restoreWorkspace(rootDir: string, single: string | null) {
  ...
  const tabs = saved.tabs.filter(
    (path) => buffers[path] || (isImagePath(path) && exists(path))
  )
  const activePath =
    saved.activePath && tabs.includes(saved.activePath)
      ? saved.activePath
      : (tabs[0] ?? null)
  return {
    activePath,
...
export function createWorkspace(deps: {
  ...
  const [buffers, setBuffers] = createStore<Record<string, FileBuffer>>(
    restored.buffers
  )
  const [views, setViews] = createSignal<string[]>(restored.tabs)
  const [activeView, setActiveView] = createSignal<string | null>(
    restored.activePath
  )
  const tabs = () => views().filter((id) => !pageKindOf(id))
  const [lastFile, setLastFile] = createSignal<string | null>(
    restored.activePath
  )
  ...
  const onPageClose = (kind: PageKind, close: () => void) => {
    pageClosers.set(kind, close)
  }
  ...
  const dirtyPaths = () =>
    Object.keys(unwrap(buffers)).filter((path) => buffers[path]?.dirty)
  ...
  const discardBuffer = (path: string) => {
    ...
    Reflect.deleteProperty(formatAbort, path)
    setBuffers(
      produce((draft) => {
        Reflect.deleteProperty(draft, path)
      })
    )
  }
  ...
  const openFile = (path: string, preview = false) => {
    ...
    if (!buffers[path] && !isImagePath(path)) {
      try {
        ...
        setNotice({ name: basename(path), reason: unreadableReason(error) })
        return
      }
    }
    ...
    setViews((prev) => {
      ...
      if (preview && slot >= 0) {
        return prev.map((p, i) => (i === slot ? path : p))
      }
      return [...prev, path]
    })
    if (preview) {
  ...
  const closeTab = (path: string, discardUnsaved = false) => {
    ...
    if (activeView() === path) {
      ...
      if (!fallback && panes.sidebar()) {
        panes.focusTree()
      }
    }
    const kind = pageKindOf(path)
    if (kind) {
      pageClosers.get(kind)?.()
      return
    }
    if (previewPath() === path) {
      setPreviewPath(null)
  ...
  const renderedPath = () => {
    const path = activePath()
    return path && isMarkdownPath(path) && renderedPaths().includes(path)
      ? path
      : null
  }
  ...
  const toggleRendered = () => {
    ...
    panes.setFocus('editor')
    say(
      rendered ? `Rendering ${basename(path)}` : `Source of ${basename(path)}`
    )
  }
  ...
  const onEditorChange = (text: string) => {
    const path = activePath()
    if (!path || !buffers[path] || buffers[path].content === text) {
      return
    }
    pinTab(path)
  ...
  const acceptConflict = (line: number, side: ConflictSide) => {
    ...
    const block = mergeConflicts().find(
      (one) => line >= one.start && line <= one.end
    )
    if (!block) {
      return say('No merge conflict on this line', 'warn')
    }
    applyReplacement(path, keepSide(buffer.content, block, side))
    editor.requestGoto(block.start, 0)
    const left = mergeConflicts().length
    const rest = left > 0 ? ` — ${plural(left, 'conflict')} left` : ''
  ...
  const replaceOverlay = (): Map<string, string> => {
    ...
    for (const [path, buffer] of Object.entries(buffers)) {
      if (buffer && (buffer.dirty || path === active)) {
        overlay.set(path, buffer.content)
      }
    }
    return overlay
  }
  ...
  const clearHold = (path: string, id: number) => {
    if (holdDisk[path]?.id === id) {
      Reflect.deleteProperty(holdDisk, path)
    }
  }
  ...
  const beginFormat = (
    ...
    return { epoch, prior, signal: ac.signal }
  }
  ...
  const clearFormatState = (path: string) => {
    ...
    Reflect.deleteProperty(holdDisk, path)
    Reflect.deleteProperty(formatWait, path)
  }
  ...
  const applyFormat = async (
    ...
    'formatted' | 'noop' | 'busy' | 'superseded' | { failed: string }
  > => {
    const error = await runFormatter(command, path, rootDir, signal)
    if (formatEpoch[path] !== epoch || signal.aborted) {
      return 'superseded'
    }
    if (error) {
      if (announce) {
        say(`Format failed: ${error}`, 'error')
      }
      return { failed: error }
    }
    if (!buffers[path]) {
    ...
    if (disk === buffer.content) {
      setBuffers(path, { encoding: written.encoding, mtime: written.mtime })
      return disk === saved ? 'noop' : 'formatted'
    }
    setBuffers(path, written)
    if (path === activePath()) {
      editor.pushEdit(disk)
    }
    git.bump()
    return 'formatted'
  }
  ...
  const formatAfterSave = (path: string, saved: string, command: string[]) => {
    ...
    formatWait[path] = (async () => {
      try {
        await task
      } catch {
        // The task reports its own failure; this handle only says when it is done.
      }
    })()
  }
  ...
  const writeBuffer = (
    ...
    if (runFormat) {
      ...
      if (command) {
        formatAfterSave(path, final, command)
      } else {
    ...
    if (final !== content && path === activePath()) {
      editor.pushEdit(final)
    }
    git.bump()
    if (!opts?.quiet) {
  ...
  const clashes = (path: string, buffer: FileBuffer): boolean => {
    ...
    if (hold && buffer.content === hold.content) {
      return false
    }
    if (!exists(path)) {
      return true
  ...
  const prepareSave = (path: string, buffer: FileBuffer): boolean => {
    ...
    if (!exists(path)) {
      setConflict({ deleted: true, disk: '', encoding: buffer.encoding, path })
      return false
    }
    let disk = ''
    let { encoding } = buffer
    ...
    setConflict({ deleted: false, disk, encoding, path })
    return false
  }
  ...
  const formatPath = (path: string): boolean => {
    ...
    if (
      buffer.dirty &&
      !writeBuffer(path, buffer.content, { quiet: true, runFormat: false })
    ) {
      return false
    }
  ...
  const formatActive = () => {
    ...
    if (!formatterFor(path, config.formatters)) {
      return say(
        'No formatter for this file — add one in Settings → Formatters',
        'warn'
      )
    }
    formatPath(path)
  ...
  const formatOpen = () => {
    const paths = tabs().filter(
      (path) => buffers[path] && formatterFor(path, config.formatters)
    )
    if (paths.length === 0) {
      return say('Nothing to format')
    }
    ...
    void (async () => {
      ...
      for (const path of paths) {
        ...
        const task = (async () => {
          ...
          const result = await applyFormat(
            path,
            saved,
            command,
            epoch,
      ...
      const bits: string[] = []
      if (done.length === 1) {
        bits.push(`Formatted ${basename(done[0]!)}`)
      } else if (done.length > 1) {
        bits.push(`Formatted ${done.length} files`)
      } else if (unchanged > 0) {
        bits.push('No formatting changes')
      }
      if (interrupted > 0) {
        bits.push(
          bits.length === 0
  ...
  const saveDirty = () => {
    ...
    return { failed, saved, skipped }
  }

  const saveDirtyOnBlur = () => {
    const { saved, skipped, failed } = saveDirty()
    if (saved > 1) {
      say(`Saved ${saved} files`)
    }
    if (skipped.length > 0) {
      say(`${CLASH_CHANGED}${skipped.join(', ')}`, 'warn')
    }
    if (failed.length > 0) {
      say(`Save failed: ${failed.join(', ')}`, 'error')
    }
  }

  const saveAll = () => {
    const { saved, skipped, failed } = saveDirty()
    if (saved === 0 && skipped.length === 0 && failed.length === 0) {
      return say('Nothing to save')
    }
    if (saved > 1) {
      say(`Saved ${saved} files`)
    }
  ...
  const syncFromDisk = (): DiskSync => {
    ...
    if (updates.length > 0) {
      setBuffers(
        produce((draft) => {
          for (const [path, buffer] of updates) {
            draft[path] = buffer
          }
        })
      )
  ...
  const applyMatchReplace = (match: Match, replacement: string) => {
    ...
    if (error) {
      return say(`Replace failed: ${error}`, 'error')
    }
    syncFromDisk()
    git.bump()
  }
  ...
  const applyProjectReplace = (
    ...
    if (result.matches === 0 && result.failed.length === 0) {
    ...
    if (result.failed.length > 0) {
      const names = result.failed
        .map((entry) => basename(entry.split(' — ')[0]!))
        .join(', ')
  ...
  const clashWarning = (sync: DiskSync): string | null => {
    ...
    if (sync.changed.length > 0) {
      parts.push(`${CLASH_CHANGED}${sync.changed.join(', ')}`)
    }
    if (sync.deleted.length > 0) {
      parts.push(`${CLASH_DELETED}${sync.deleted.join(', ')}`)
    }
  ...
  const remapPaths = (remap: (path: string) => string) => {
    ...
    if (active) {
      setLastFile(remap(active))
    }
    const view = activeView()
    if (view) {
      setActiveView(remap(view))
    }
    const preview = previewPath()
    if (preview) {
      setPreviewPath(remap(preview))
    }
    setRenderedPaths((prev) => prev.map(remap))
  }

  const autoSaveLeft = (path: string) => {
    if (!config.autoSaveOnBlur) {
      return
    }
    if (!tabs().includes(path)) {
      return
    ...
    if (autoSave(path) === 'skipped') {
      say(`${CLASH_CHANGED}${basename(path)}`, 'warn')
    }
  }
