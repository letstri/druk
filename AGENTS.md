...
list of named things uses, branches included: an item may carry a `note` for its
right-hand column and `current` for the `*` mark, and the mark's column is drawn
only where something is marked — then apply/pop/drop, drop behind a
...
all changes, `a` in the panel), Enter opens the changed file itself (a folder
row folds instead), and the page is a *tab* like any other — one `Changes` tab
however often it is asked for, left on the strip when a file is opened and closed
with its × or Ctrl+W. Tab into the page lights the current file's header (a
...
typescript (ts/tsx/js/jsx), json, markdown, html, css, yaml, toml, dotenv and diff.
Those are the languages a first run has to highlight with no network — dotenv among
them because a `.env` is a file every project has and nobody thinks to install a
language extension for, and diff because a `.patch` under `patches/` is the same
kind of file: every lockfile-patching project has one and nobody goes looking for
an extension to read it.
...
both routes — `/` and `/extensions` — and hands the result to Nitro, which writes the
layout of whichever host it is building on — `.output/` locally, `.vercel/output/` on
Vercel. The `nitro()` plugin in `vite.config.ts` is what makes deployment work at all:
Start on its own writes `dist/client` and `dist/server`, and Vercel serves `dist/`, so
the site 404s without it.
...
That import is what makes the extensions page the catalog itself rather than a list
kept in step by hand: `bun run extensions` regenerates the JSON and the page follows,
counts and all. Only what a manifest cannot say is written out there — which eight ids
are preinstalled (`BUILTIN`, mirroring `src/extensions/builtin.ts`), since the catalog
does not mark them and the page would otherwise offer a first-run user what they
already have.
The page is styled as a terminal session
(GitHub Dark, druk's default theme); the editor mock in `src/components/EditorMock.tsx` is
...
`src/routeTree.gen.ts` is generated by the dev server/build and gitignored — a new route
is invisible to `tsc` until a `bun run dev` or `bun run build` has regenerated it, which
reads as a type error on the `Link` that points at it.
`web/public/` is served at the site root: `og.png` (the social card, designed in Paper and
exported at 1200x630 — the size every crawler crops to), `favicon.svg`, `robots.txt` and a
hand-written `sitemap.xml` that lists the two routes, so adding a third means adding it
there too. The shared meta — description, Open Graph, Twitter card, the
`SoftwareApplication` JSON-LD — is in `__root.tsx`; each route adds its own `canonical` and
`og:url`, which the root cannot spell since it has no path. The absolute URLs in all of
that are why a preview build's card still points at production.
...
bun run test             # unit + UI, one file per process, half the cores at a time (~2 min)
...
| theme | a `themes` entry in a market manifest — `extensions/<family>/extension.json`, one extension per palette family (catppuccin carries its four flavors), then `bun run extensions`. Only `dark` and `light` are built in, in `src/themes/`, because the defaults name them. Chrome roles that are a *relationship* between two colours (`border`, `sidebarBg`, `solidBg`) are derived in `colorsFor` there and are never listed by a theme. A `syntax` map lists *root* scopes and the sub-scopes it wants to differ: `styleIdForGroup` walks `type.builtin` → `type` by itself, and `FALLBACK_GROUP` (`src/languages/highlight.ts`) is what saves a root the theme never heard of — `attribute` → `property`, `constructor` → `function`, `namespace` → `type`. A group nothing resolves to paints as plain text, which is why that walk decides membership with `getStyle` and not with `getStyleId`: the native style table invents an id for any name it is asked about, so `getStyleId` never answers null. Neither walk reaches the *markdown renderable*, which asks the native table by name and gets nothing for a dotted group it does not hold: `markdownGroups` there registers the levelled heading captures (`markup.heading.1`…`.6`), `markup.raw.block`, the list checkboxes, `conceal` and `default` off the theme's own `markup.*`, without which a rendered heading paints as plain text and a table cell's prose takes `TextTable`'s white — invisible on a light theme |
...
| scroll that must survive a layout pass | `retryFrames` (`src/ui/list.ts`) — a scrollbox clamps an offset against the height it still has, and the layout pass writes the new one later than any macrotask, so one shot lands short. The callback reports whether it landed; `defer` waits a macrotask first, for a reveal that grows the list. A sidebar view is unmounted whenever another takes the slot, so `createScrollList` takes a key and keeps that view's offset in `kept`, restoring it on the next mount unless a reveal has already moved the list — which is what stops a walk between the tree and the git panel from re-scrolling both every time. `forgetScroll` drops the offsets where a fresh look is wanted instead: closing the sidebar (`toggleSidebar`) and the workspace remount, both of which then centre the cursor again. `keptScroll`/`keepScroll` are the same store for a page that is a tab and so unmounts behind every other one — `ChangesView` restores its offset on mount and re-measures once it lands, the sticky header being read off renderables, and skips the reveal for the file it already served, or coming back to the tab would throw the reader at that file's first change |
...
- **A test process starts with the preinstalled extensions and nothing else.**
  ...
  toml, dotenv and diff highlight as they do on a real first run. Anything else — Go, Python,
...
- **A git fixture opens with `initRepo(dir)` (`test/repo.ts`), never its own init.**
  It sets the identity *and* turns `commit.gpgsign` off, without which the suite
  stops at its first commit on a machine with signing on globally — which is how
  eighteen of the thirty-three hand-written preambles were spelled. `git(dir, …)`
  is there too. Each fixture still builds its own files and commits; only the
  preamble is shared.
